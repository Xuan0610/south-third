...
function isNotValidGender(gender) {
  return typeof gender !== 'string' || gender.trim().length > 10 || gender === '';
}

function isNotValidBirthday(dateStr) {
  const birthdayRegex = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/;
  if (!birthdayRegex.test(dateStr)) return true;
...
function isNotValidTaiwanMobile(phone) {
  const regex = /^09\d{8}$/;
...
const taiwanAreas = {
  ...
  '彰化縣': ['彰化市', '員林市', '和美鎮', '鹿港鎮', '溪湖鎮', '二林鎮', '田中鎮', '北斗鎮', '花壇鄉', '芬園鄉', '大村鄉', '永靖鄉', '伸港鄉', '線西鄉', '福興鄉', '秀水鄉', '埔心鄉', '埔鹽鄉', '大城鄉', '芳苑鄉', '竹塘鄉', '溪州鄉', '社頭鄉', '二水鄉', '田尾鄉', '埤頭鄉'],
...
function isNotValidTaiwanAddressAdvanced(address) {
